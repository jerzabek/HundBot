...
import ga.jarza.hundbot.main.commands.moderation.Moderation;
...
public class ChatEvents {
  ...
  @EventSubscriber public void onMessageReceived(MessageReceivedEvent event) {
    ...
    if (in <= 9) {
      if (!Util.botCommand(event.getMessage().getContent()) && event.getMessage().getFormattedContent().length() > 0) {
        ...
        try {
          DataManager.saveMeme(new Meme(event.getMessage().getFormattedContent(),event.getAuthor().getLongID(), event.getGuild().getLongID(), Util.formatter.format(event.getMessage().getTimestamp()), atts));
  ...
  @EventSubscriber public void onReady(ReadyEvent event) {
    MainBot.cli.changePresence(StatusType.ONLINE, ActivityType.PLAYING, Util.prefix + "help | " + MainBot.cli.getUsers().size() + " users");
