...
import ga.jarza.hundbot.main.commands.moderation.Permission;
import ga.jarza.hundbot.main.commands.music.GuildMusicManager;
    ...
    ChatCommands.adminMap.put("game", (MessageReceivedEvent event, List<String> args) -> {
      if (event.getAuthor().getLongID() == Util.jarza) {
        if (args.size() != 0) {
          ...
          MainBot.cli.changePresence(StatusType.ONLINE, ActivityType.WATCHING, " you sleep.");
        }
      }
    });
...
//      System.out.println(musicManager.scheduler.getQueuedTracks().size());
    ...
    commandMap.put("serverinfo", (event, args) -> {
      ...
      e.appendField("Server created:", Util.formatter.format(g.getCreationDate()), false); //.format(DateTimeFormatter.ofPattern("dd/MM/yyyy - HH:mm"))
    ...
    commandMap.put("info", (event, args) -> {
      ...
      e.appendField("Joined server:", event.getGuild().getJoinTimeForUser(g).toString(), true); //.format(DateTimeFormatter.ofPattern("dd/MM/yyyy - HH:mm"))
      e.appendField("User created account:", g.getCreationDate().toString(), true); //.format(DateTimeFormatter.ofPattern("dd/MM/yyyy - HH:mm"))
...
//          Util.sendMessage(event, val + "/" + initial + "/" + wanted);
    ...
    adminMap.put("makepremium",  (MessageReceivedEvent event, List<String> args) -> {
      ...
      if(event.getMessage().getMentions().isEmpty()){
        ...
        DataManager.setUserPremium(event.getMessage().getMentions().get(0).getLongID(), des, event.getMessage().getTimestamp().toString());
        if(des)
      ...
      e.withFooterText(event.getMessage().getTimestamp().toString());
      Util.sendMessage(event, e.build());
    });
